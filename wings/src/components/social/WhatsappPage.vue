...
<div class="ui form">
  <div v-if="!dedaloRequested">
    <div v-if="choosedMode" class="field" v-bind:class="{ error: errors.badInput }">
      ...
      <div v-if="!codeRequested" class="not-code-exp">
        <div class="ui compact message info no-margin-top">
          <div class="content">
            <div class="header">{{$t('whatsapp.wait')}}</div>
            <p>{{$t('whatsapp.we_are_sending_whatsapp_code')}}</p>
          </div>
        </div>
        <div v-show="openBtn" @click="redirectAuth()" class="ui green button big">
          <i class="whatsapp icon"></i>
          {{$t('whatsapp.open')}}
        </div>
      </div>
      ...
      <div v-if="codeRequested">
        <div class="field">
          <label :style="textStyle">{{ $t("whatsapp.code") }}</label>
          <div class="ui big left icon input">
            <input
              readonly
              v-model="authCode"
              ...
              :placeholder="$t('whatsapp.insert_your_code')"
            />
    ...
    <div v-if="dedaloRequested">
      <div
        v-if="!authorized && !errors.dedaloError"
        class="ui active centered inline text loader"
        :style="textStyle"
      >
        {{
        $t("whatsapp.auth_progress") }}...
      </div>
      <div v-if="authorized" class="ui icon positive message">
        <i class="check icon"></i>
        <div class="content">
          <div class="header">{{ $t("whatsapp.auth_success") }}</div>
        </div>
      </div>
      <div
...
export default {
  ...
  data: function() {
    return {
      ...
      openBtn: this.$route.query.temp || false,
      dedaloRequested: false,
      authNum: this.$route.query.num || "",
  ...
  methods: {
    ...
    isDisabled: function() {
      return this.authNum.length == 0 || this.authCode.length == 0;
    },
