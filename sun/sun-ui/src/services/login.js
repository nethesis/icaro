var LoginService = {
  methods: {
    execLogin(body, success, error) {
      this.$http.post(this.$root.$options.api_scheme + this.$root.$options.api_host + '/api/login', body).then(success, error);
    },
    execLogout(success, error) {
      this.$http.post(this.$root.$options.api_scheme + this.$root.$options.api_host + '/api/logout', {}, {
        headers: {
