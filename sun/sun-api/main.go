...
func DefineAPI(router *gin.Engine) {
	...
	api.GET("/userinfo", methods.UserInfo)
	...
	api.POST("/auth/oidc/device/start", methods.OIDCDeviceStart)
	api.POST("/auth/oidc/device/poll", methods.OIDCDevicePoll)
	...
	api.Use(middleware.AAWall)
	{
		...
		account_integrations := api.Group("/account_integrations")
		{
			account_integrations.GET("/:account_id", methods.GetAccountIntegrations)
			account_integrations.POST("/:account_id/:integration_id", methods.CreateAccountIntegrations)
			account_integrations.DELETE("/:account_id/:integration_id", methods.DeleteAccountIntegrations)
		}

		hotspot_integrations := api.Group("/hotspot_integrations")
		{
			hotspot_integrations.GET("/:hotspot_id", methods.GetHotspotIntegrations)
			hotspot_integrations.PUT("/:hotspot_id/:integration_id", methods.UpdateHotspotIntegrations)
			hotspot_integrations.DELETE("/:hotspot_id/:integration_id", methods.DeleteHotspotIntegrations)
		}
		...
		stats := api.Group("/stats")
		{
			...
			smsStats.PUT("/accounts/:account_id", methods.UpdateSMSThresholdForAccount)
		...
		vouchers := api.Group("/vouchers")
		{
			...
			vouchers.DELETE("/hotspot/:hotspot_id", methods.DeleteAllVouchers)
			vouchers.DELETE("/voucher/:voucher_id", methods.DeleteVoucher)
		}
