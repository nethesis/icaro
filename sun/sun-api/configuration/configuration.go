...
func Init(ConfigFilePtr *string) {
	...
	if os.Getenv("EMAIL_FROM_NAME") != "" {
		Config.Endpoints.Email.FromName = os.Getenv("EMAIL_FROM_NAME")
	}
	...
	if os.Getenv("CAPTIVE_TITLE_COLOR") != "" {
		Config.CaptivePortal.TitleColor = os.Getenv("CAPTIVE_TITLE_COLOR")
	}
